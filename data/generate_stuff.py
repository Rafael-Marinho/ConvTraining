...
# Creates parafernalia for the training, based on amount of classes:
classes = open("data/train/classes.txt").read()

# Classes file:
names = open("data/classes.names", 'w')
names.write(classes.replace(' ', '_'))

# Directives for the training process:
data = open("data/directives.data", 'w')
data.write('''classes = CLASSES
train = data/train.txt
valid = data/validation.txt
names = data/classes.names
backup = models'''.replace("CLASSES", str(len(classes.splitlines()))))

names.close()
data.close()


# YOLOv4 architecture's variables:
...
# The YOLOv4 Full architecture itself:
CFG = '''[net]
batch=64
subdivisions=16
# Training
...
# The YOLOv4 Tiny architecture as well:
CFG = '''[net]
# Testing
#batch=1
#subdivisions=1
...
# Get all the images for the validation process and insert their relative path
# inside a file:
image_files = []
...
# Get all the images for the training process and insert their relative path
# inside a file:
image_files = []
